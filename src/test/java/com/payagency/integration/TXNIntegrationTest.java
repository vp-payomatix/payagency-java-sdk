...
public class TXNIntegrationTest {
    ...
    public void testGetTransactions() throws Exception {
        ...
        if (response.getData() != null) {
            ...
            // Validate first transaction if available
            if (!response.getData().isEmpty()) {
                TransactionsOutput.Transaction firstTxn = response.getData().get(0);
                System.out.println("  First transaction:");
                System.out.println("    Transaction ID: " + firstTxn.getTransactionId());
                System.out.println("    Amount: " + firstTxn.getAmount() + " " + firstTxn.getCurrency());
                System.out.println("    Status: " + firstTxn.getStatus());
                System.out.println("    Customer: " + firstTxn.getFirstName() + " " + firstTxn.getLastName());
                System.out.println("    Email: " + firstTxn.getEmail());
                System.out.println("    Transaction Date: " + firstTxn.getTransactionDate());
                
                // Validate required fields
                assertNotNull(firstTxn.getTransactionId(), "Transaction ID should not be null");
                assertNotNull(firstTxn.getStatus(), "Status should not be null");
            }
        }
        
        if (response.getMeta() != null) {
            System.out.println("  Pagination info:");
            System.out.println("    Has next page: " + response.getMeta().isHasNextPage());
            System.out.println("    Has previous page: " + response.getMeta().isHasPreviousPage());
            System.out.println("    Total count: " + response.getMeta().getTotalCount());
            System.out.println("    Next cursor: " + response.getMeta().getNextCursor());
            System.out.println("    Previous cursor: " + response.getMeta().getPrevCursor());
        }
    }
    ...
    public void testGetTransactionsWithoutDateFilter() throws Exception {
        ...
        if (response.getMeta() != null) {
            System.out.println("  Total transactions: " + response.getMeta().getTotalCount());
        }
    }
    ...
    public void testGetWalletTransactions() throws Exception {
        ...
        if (response.getData() != null) {
            System.out.println("  Number of wallet transactions: " + response.getData().size());
            
            // Validate first wallet transaction if available
            if (!response.getData().isEmpty()) {
                TransactionsOutput.Transaction firstTxn = response.getData().get(0);
                System.out.println("  First wallet transaction:");
                System.out.println("    Transaction ID: " + firstTxn.getTransactionId());
                System.out.println("    Amount: " + firstTxn.getAmount() + " " + firstTxn.getCurrency());
                System.out.println("    Status: " + firstTxn.getStatus());
                System.out.println("    Transaction Type: " + firstTxn.getTransactionType());
                
                // Validate wallet-specific data
                if (firstTxn.getConvertedAmount() != null && firstTxn.getConvertedCurrency() != null) {
                    System.out.println("    Converted: " + firstTxn.getConvertedAmount() + " " + firstTxn.getConvertedCurrency());
                }
            }
        }
        
        if (response.getMeta() != null) {
            System.out.println("  Wallet transactions pagination:");
            System.out.println("    Total count: " + response.getMeta().getTotalCount());
            System.out.println("    Has next page: " + response.getMeta().isHasNextPage());
        }
    }
